...
class BaseTransformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.cnn_extractor = Vox2DCNN(self.attn_size, self.dropout, out_len=self.visual_seq_len)
        
        self.pos_encoding = LearnablePositionalEncoding(self.attn_size, self.dropout, max_len=max_length)
    ...
    def forward_train(self, x_in, actions_in, n_actions):
        ...
        all_embeddings = th.cat((cmd_token_embeddings, param_token_embeddings), dim = 2)
        all_embeddings = all_embeddings.reshape(batch_size, max_action_size, -1)
